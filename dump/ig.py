...
def menu():
    ...
    if aset is None:
        ...
        return
    os.system('clear')
    prints(Panel.fit(f"[bold white] Usahakan Target Di Atas 1k Follower"))
...
def dumps(kuki, typess, xyz=[]):
    if 'csrftoken' not in str(kuki):
        try:
            response = requests.get('https://www.instagram.com/data/shared_data/', cookies=kuki)
            response.raise_for_status()  # Pastikan request berhasil
            memek = response.json()
            token = memek.get('config', {}).get('csrf_token', None)
            if token:
                kuki['cookie'] += f';csrftoken={token};'
            else:
                console.print("[red]Gagal mendapatkan CSRF token![/red]")
                exit()
        except Exception as e:
    ...
    console.print(Panel.fit("[bold white] Masukan Target Username Anda", style="bold blue"))      

    users = input("└──╭➣ Targets User : ").split(',')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(process_user, y, kuki, typess, xyz) for y in users]
        hasil_dump.extend([future.result() for future in concurrent.futures.as_completed(futures)])

    if os.path.exists('dump.txt'):
        with open('dump.txt', 'r') as file:
            usernames = [line.split('|')[0].strip() for line in file.readlines()]
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(process_user, x, kuki, typess, xyz) for x in usernames]
            hasil_dump.extend([future.result() for future in concurrent.futures.as_completed(futures)])

        console.print(Panel.fit("[bold white] Proses Dump ke-2 Selesai", style="bold green"))

    if os.path.exists('dump.txt'):
        with open('dump.txt', 'r') as file:
            usernames2 = [line.split('|')[0].strip() for line in file.readlines()]
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(process_user, z, kuki, typess, xyz) for z in usernames2]
            hasil_dump.extend([future.result() for future in concurrent.futures.as_completed(futures)])

        console.print(Panel.fit("[bold white] Proses Dump ke-3 Selesai!", style="bold green"))
...
def process_user(user, kuki, typess, xyz):
    try:
        req = requests.get(f'https://www.instagram.com/{user}/', cookies=kuki)
        req.raise_for_status()  # Pastikan request berhasil

        match = re.search(r'"user_id":"(\d+)"', req.text)
        if match:
            uid = match.group(1)
            if uid not in xyz:
                xyz.append(uid)
            ...
            return f"{user} | {uid} | {mode}"
        else:
            console.print(f"[yellow]User ID tidak ditemukan untuk {user}[/yellow]")
            return f"{user} | User ID tidak ditemukan"
    except requests.exceptions.RequestException as e:
        console.print(f"[red]Error saat memproses {user}: {e}[/red]")
        return f"{user} | Error"
